...
export const userStore = create<UserStore>((set) => ({
    user: { name: "empty", email: "empty" , password:'empty' }, // Initial state
    ...
    setUser: (updatedUser: User) => set((state: UserStore) => ({ ...state, user: updatedUser })), 

    //to change all the value in the existing state you set it by defining inside the set only
    removeUser : () =>set({user :{name:'empty' , email:'empty'}})
}));
